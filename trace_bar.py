...
from_path = './PW_a120_w040/'
to_path   = './PW_a120_w040/'
part_name='proton'
part_mass=1836.0*1

data = sdf.read(from_path+"0018.sdf",dict=True)
grid_x = data['Grid/Particles/'+part_name].data[0]/1e-6
grid_y = data['Grid/Particles/'+part_name].data[1]/1e-6
...
part13_id = part13_id[(px>0)&(abs(grid_y)<12)&(abs(theta)<30)]
#part13_id = part13_id[abs(grid_y)<12]
...
px_d = np.zeros([part_id.size,int((stop-start)/step)+1])
py_d = np.zeros([part_id.size,int((stop-start)/step)+1])
xx_d = np.zeros([part_id.size,int((stop-start)/step)+1])
yy_d = np.zeros([part_id.size,int((stop-start)/step)+1])
for n in range(start,stop+step,step):
    #### header data ####
    data = sdf.read(from_path+str(n).zfill(4)+".sdf",dict=True)
    px = data['Particles/Px/'+part_name].data/(part_mass*m0*v0)
    ...
    for ie in range(part_id.size):
        px_d[ie,int((n-start)/step)] = px[temp_id==part_id[ie]]
        py_d[ie,int((n-start)/step)] = py[temp_id==part_id[ie]]
        xx_d[ie,int((n-start)/step)] = grid_x[temp_id==part_id[ie]]
        yy_d[ie,int((n-start)/step)] = grid_y[temp_id==part_id[ie]]
    print('finised '+part_name+' '+str(round(100.0*(n-start+step)/(stop-start+step),4))+'%')

np.savetxt(to_path+'wrap_bar_'+part_name+'_px.txt',px_d)
np.savetxt(to_path+'wrap_bar_'+part_name+'_py.txt',py_d)
np.savetxt(to_path+'wrap_bar_'+part_name+'_xx.txt',xx_d)
np.savetxt(to_path+'wrap_bar_'+part_name+'_yy.txt',yy_d)
