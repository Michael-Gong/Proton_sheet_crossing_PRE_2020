...
if __name__ == '__main__':
  ...
  for n in range(start,stop+step,step):   
      ...
      plt.subplot(1,2,1)
      plt.scatter(px_d[:,n-start], py_d[:,n-start], c=xx_d[:,0], cmap='rainbow', norm=colors.Normalize(vmin=0, vmax=0.3) , s=marker_size, marker='.',alpha=0.8,label=part_name+' $p_y-p_x$',zorder=1,lw=0)
...
#      plt.legend(loc='best',fontsize=14)
      plt.xlim(-0.9,0.9)
      plt.ylim(-0.9,0.9)
      plt.xlabel('$p_x\ [m_ic]$',fontdict=font)
      plt.ylabel('$p_y\ [m_ic]$',fontdict=font)
      plt.xticks(fontsize=font_size); 
      plt.yticks(fontsize=font_size);

      plt.subplot(1,2,2)
      plt.scatter(yy_d[:,n-start], py_d[:,n-start], c=xx_d[:,0], cmap='rainbow', norm=colors.Normalize(vmin=0, vmax=0.3) , s=marker_size, marker='.',alpha=0.8,label=part_name+' $y-p_y$',zorder=1,lw=0)
      cbar=plt.colorbar(pad=0.01,ticks=np.linspace(0, 0.3, 4))
      cbar.ax.tick_params(labelsize=font2['size']) 
      #cbar.ax.set_yticklabels(cbar.ax.get_yticklabels(),fontsize=font2['size'])        
      cbar.set_label('$x|_{t=0}$',fontdict=font2)        
...
#      plt.title('Au51 at '+str(round(time1/1.0e-15,2))+' fs',fontdict=font)
      ...
      fig.savefig(to_path+part_name+'_px_py_pos'+str(n).zfill(4)+'.png',format='png',dpi=80)
      plt.close("all")
      print('finised '+to_path+part_name+'_px_py_pos'+str(n).zfill(4)+'.png')
