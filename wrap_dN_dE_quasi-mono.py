...
if __name__ == "__main__":
  ...
  dist_x1, den1 = pxpy_to_energy(ek,ww)
  plt.plot(dist_x1,den1,color='crimson',linewidth=3,label='$\sigma_0=1\mu m;\ a_0=122$')

  data = sdf.read('./PW_w020/'+str(13).zfill(4)+".sdf",dict=True)
  time=data['Header']['time']
  px = data['Particles/Px/'+name].data/(mass*m0*v0)
  py = data['Particles/Py/'+name].data/(mass*m0*v0)
  grid_y = data['Grid/Particles/'+name].data[1]/1e-6
  ...
  ww=ww[(px>0)&(abs(grid_y)<2)&(abs(theta)<30)]
  dist_x1, den1 = pxpy_to_energy(ek,ww)
  plt.plot(dist_x1,den1,color='seagreen',linewidth=3,label='$\sigma_0=2\mu m;\ a_0=61$')
...
#  plt.plot(dist_x1,den1,color='mediumblue',linewidth=3,label='$\sigma_0=4\mu m;\ a_0=31$')
  ...
  ww=ww[(px>0)&(abs(grid_y)<2)&(abs(theta)<30)]
  dist_x1, den1 = pxpy_to_energy(ek,ww)
  plt.plot(dist_x1,den1,color='purple',linewidth=3,label='$\sigma_0=8\mu m;\ a_0=15$')
...
#    plt.ylim(1e1,1e5)
  ...
  fig.savefig('wrap_dN_dE_quasi-mono.png',format='png',dpi=160)
  plt.close("all")
  print('wrap_dN_dE_quasi-mono.png')
