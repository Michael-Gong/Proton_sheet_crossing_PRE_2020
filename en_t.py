...
def pxpy_to_energy(gamma, weight):
    binsize = 400
    en_grid = np.linspace(0.5,399.5,400)
    en_bin  = np.linspace(0,400,401)
...
if __name__ == "__main__":
  ...
  for i in [10,20,40,80,120]:
      from_path='./PW_w'+str(i).zfill(3)+'/'
      to_path='./PW_w'+str(i).zfill(3)+'_fig/'
      ...
      for n in range(start,stop+step,step):
        ...
        header=data['Header']
        time_s[n]=header['time']/1e-15
        mass = 1836.
       # name = 'subset_trace_p/t_proton'
        name = 'proton'
        px = data['Particles/Px/'+name].data/(mass*m0*v0)
        py = data['Particles/Py/'+name].data/(mass*m0*v0)
        grid_y = data['Grid/Particles/'+name].data[1]/1e-6
    ...
    #        ww_x_d = ww[ (grid_x>5) & (abs(grid_y)<3.) & (work_x>work_y)]
        ...
        en_cut[n]=np.max(dist_x1[den1>1e5])
     
      np.savetxt(to_path+'en_cut.txt',en_cut)
      np.savetxt(to_path+'en_cut_t.txt',time_s)
     
      plt.subplot(1,1,1)
      
      plt.plot(time_s,en_cut,linestyle='None',marker='^', fillstyle='none', markersize=10, mew=2, linewidth=3, color='crimson', label='PIC Simulation')
      plt.xlabel('$t$ [fs]',fontdict=font)
      plt.ylabel(r'$\varepsilon_i$'+' [MeV]',fontdict=font)
    ...
    #  plt.title('t='+str(round(time/1.0e-15,0))+' fs',fontdict=font)
      plt.subplots_adjust(left=0.12, bottom=0.12, right=0.98, top=0.97, wspace=0.011, hspace=0.16)
      fig = plt.gcf()
      fig.set_size_inches(10, 5.)
      fig.savefig('w'+str(i).zfill(3)+'_en_t.png',format='png',dpi=160)
      plt.close("all")
      print('w'+str(i).zfill(3)+'_en_t.png')
