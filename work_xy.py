...
#      plt.hist2d(work_x, work_y, bins=(200, 200), range=[[-200,800],[-200,800]], cmap='cubehelix_r', weights=weight, normed=False)
      plt.hist2d(work_x, work_y, bins=(200, 200), range=[[-700,300],[-300,700]], cmap=mycolor_jet, weights=weight, normed=False,norm=colors.LogNorm(vmin=1e5,vmax=1e11))
      cbar=plt.colorbar(pad=0.01)
      cbar.set_label(r'$dN/(dW_xdW_y)$'+' [A.U.]',fontdict=font)
      cbar.ax.tick_params(labelsize=font2['size'])
      plt.xlim(-700,300)
      plt.ylim(-300,700)
      plt.xlabel('$W_x$ [MeV]',fontdict=font)
      plt.ylabel('$W_y$ [MeV]',fontdict=font)
      ...
      fig.savefig(to_path+'work_'+str(n).zfill(4)+'_xy.png',format='png',dpi=160)
      plt.close("all")
      print(to_path+'work_'+str(n).zfill(4)+'_xy.png')
