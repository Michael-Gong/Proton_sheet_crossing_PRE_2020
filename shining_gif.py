...
#          ion_px = data['Particles/Px/proton'].data/(1836*m0*v0)
    ...
    if n<=400:
        plt.xticks([-2,0,2,4],fontsize=font_size); 
        plt.yticks([-2,1,0,1,2],fontsize=font_size);
    if n>680: 
    ...
    fig.savefig(to_path+'shining_'+str(n).zfill(4)+'.png',format='png',dpi=80)
    plt.close("all")
    print(to_path+'shining_'+str(n).zfill(4)+'.png')
