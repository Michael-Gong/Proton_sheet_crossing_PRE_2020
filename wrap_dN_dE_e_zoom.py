...
def pxpy_to_energy(gamma, weight):
    binsize = 100
    en_grid = np.linspace(0,200,100)
    en_bin  = np.linspace(0,200,101)
...
if __name__ == "__main__":
  ...
  for n in [5]:
      ...
      ww = data['Particles/Weight/'+name].data*4e-6
      theta = np.arctan2(py,px)*180.0/np.pi
      ek=ek[(px>0)&(abs(grid_y)<12)]
      ww=ww[(px>0)&(abs(grid_y)<12)]
      dist_x1, den1 = pxpy_to_energy(ek,ww)
      plt.scatter(dist_x1,den1,c='red',marker='o',s=marker_s,label='$\sigma_0=1\mu m$',edgecolors='darkred',linewidth='2.5',alpha=1,zorder=1)
    #  plt.plot(dist_x1,den1,color='crimson',linewidth=3,label='$\sigma_0=1\mu m$')
      ...
      ww=ww[(px>0)&(abs(grid_y)<12)]
      dist_x1, den1 = pxpy_to_energy(ek,ww)
      plt.scatter(dist_x1,den1,c='limegreen',marker='o',s=marker_s,label='$\sigma_0=2\mu m$',edgecolors='darkgreen',linewidth='2.5',alpha=1,zorder=1)
    #  plt.plot(dist_x1,den1,color='seagreen',linewidth=3,label='$\sigma_0=2\mu m$')
      ...
      ww=ww[(px>0)&(abs(grid_y)<12)]
      dist_x1, den1 = pxpy_to_energy(ek,ww)
      plt.scatter(dist_x1,den1,c='dodgerblue',marker='o',s=marker_s,label='$\sigma_0=4\mu m$',edgecolors='mediumblue',linewidth='2.5',alpha=1,zorder=1)
    #  plt.plot(dist_x1,den1,color='mediumblue',linewidth=3,label='$\sigma_0=4\mu m$')
      ...
      ww=ww[(px>0)&(abs(grid_y)<12)]
      dist_x1, den1 = pxpy_to_energy(ek,ww)
      plt.scatter(dist_x1,den1,c='mediumpurple',marker='o',s=marker_s,label='$\sigma_0=8\mu m$',edgecolors='purple',linewidth='2.5',alpha=1,zorder=1)
    #  plt.plot(dist_x1,den1,color='black',linewidth=3,label='$\sigma_0=8\mu m$')
      print(den1)
    
      data = sdf.read('./PW_w120/'+str(n).zfill(4)+".sdf",dict=True)
      time=data['Header']['time']
      px = data['Particles/Px/'+name].data/(mass*m0*v0)
      py = data['Particles/Py/'+name].data/(mass*m0*v0)
      grid_y = data['Grid/Particles/'+name].data[1]/1e-6
      ...
      ww=ww[(px>0)&(abs(grid_y)<12)]
      dist_x1, den1 = pxpy_to_energy(ek,ww)
      plt.scatter(dist_x1,den1,c='grey',marker='o',s=marker_s,label='$\sigma_0=12\mu m$',edgecolors='black',linewidth='2.5',alpha=1,zorder=1)
    #  plt.plot(dist_x1,den1,color='black',linewidth=3,label='$\sigma_0=8\mu m$')
      print(den1)
    
      plt.xlabel(r'$\varepsilon_e$'+' [MeV]',fontdict=font)
      plt.ylabel(r'$dN/d\varepsilon_e$'+' [A.U.]',fontdict=font)
      ...
      fig.savefig('wrap_dN_dE_e_zoom'+str(n).zfill(4)+'.png',format='png',dpi=160)
      plt.close("all")
      print('wrap_dN_dE_e_zoom'+str(n).zfill(4)+'.png')
