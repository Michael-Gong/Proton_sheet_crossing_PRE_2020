  ...
  for i in [10,20,40,80,120]:
      ...
      for n in range(start,stop+step,step):
       ...
       # name = 'subset_trace_p/t_proton'
        ...
        ww=ww[ek>0.51*a0]
        ek=ek[ek>0.51*a0]
        ave_en = np.sum(ww*ek)/np.sum(ww)
        en_tem[n]=ave_en-0.51*a0
     
      np.savetxt(to_path+'en_tem.txt',en_tem)
      np.savetxt(to_path+'en_tem_t.txt',time_s)
     
      plt.subplot(1,1,1)
      
      plt.plot(time_s,en_tem,linestyle='None',marker='o', fillstyle='none', markersize=10, mew=2, linewidth=3, color='blue', label='PIC Simulation')
    ...
    #  plt.title('t='+str(round(time/1.0e-15,0))+' fs',fontdict=font)
      ...
      fig.savefig('w'+str(i).zfill(3)+'_temp_e_t.png',format='png',dpi=160)
      plt.close("all")
      print('w'+str(i).zfill(3)+'_temp_e_t.png')
