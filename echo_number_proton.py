...
if __name__ == "__main__":
  ...
  for direct in ['010','020','040','080','120']:
      data = sdf.read('./PW_w'+direct+'/'+str(n).zfill(4)+".sdf",dict=True)
      ...
      ww = data['Particles/Weight/'+name].data*4e-6
      theta = np.arctan2(py,px)*180.0/np.pi
      ek=ek[(px>0)&(abs(grid_y)<2)&(abs(theta)<3)]
      ww=ww[(px>0)&(abs(grid_y)<2)&(abs(theta)<3)]

      print(direct+':')
      for en_th in [1,10,100,200]:
          print(en_th,np.sum(ww[ek>en_th])/(1e-9/1.6e-19))
