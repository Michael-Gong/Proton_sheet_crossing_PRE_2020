...
##end for norm colorbar####
  ...
  for dir_l in ['w010','w020','w040','w080','w120','a120_w020','a120_w040']:
      from_path='./PW_'+dir_l+'/' 
      to_path  ='./PW_'+dir_l+'_fig/' 
    #  youwant = ['electron_x_px','electron_density','electron_en','electron_theta_en','ey'] #,'electron_ekbar']
      ...
      #youwant =  ['ex','ey_averaged','ey','bz_averaged','electron_no_density','electron_no_ekbar']#,'Subset_high_e_density','Subset_high_e_ekbar']
      youwant = ['ex_averaged','ex']
    #  youwant =  ['ex','ey','ey_averaged','bz','electron_s_density','electron_s_ekbar','ion_s_density','ion_s_ekbar','electron_s_polarize_x','electron_s_polarize_y','electron_s_polarize_z','ion_s_polarize_x','ion_s_polarize_y','ion_s_polarize_z']
      ...
      for n in range(start,stop+step,step):
        ...
        for name in youwant:
            ...
            if (name[0:2] == 'ex') or (name[0:2] == 'ey') or (name[0:2] == 'ez'):
                    ...
                    eee=np.max([-np.min(ex.T),np.max(ex.T)])
                    levels = np.linspace(-eee, eee, 40)
                    plt.contourf(X[::4,::4], Y[::4,::4], ex.T[::4,::4], levels=levels, cmap=cm.seismic)
                    ...
                    plt.title(name+' at '+str(round(time/1.0e-15,6))+' fs',fontdict=font)
                    plt1 = plt.twinx()
                    plt1.plot(x,ex[:,y.size//2],'-y',linewidth=2.5)
                    plt1.set_ylabel('Normalized '+name)
                    fig = plt.gcf()
                    fig.set_size_inches(12, 7)
                    fig.savefig(to_path+name+str(n).zfill(4)+'.png',format='png',dpi=100)
                    ...
                    if np.min(pol.T) == np.max(pol.T):
                        continue
                    eee=np.max([-np.min(pol.T),np.max(pol.T)])
                    levels = np.linspace(-eee, eee, 40)
                    plt.contourf(X, Y, pol.T, levels=levels, cmap=cm.bwr)
                    #### manifesting colorbar, changing label and axis properties ####
                    cbar=plt.colorbar(ticks=[-eee, -eee/2, 0, eee/2, eee])
                    cbar.set_label(name,fontdict=font)        
                    plt.xlabel('X [$\mu m$]',fontdict=font)
                    plt.ylabel('Y [$\mu m$]',fontdict=font)
                    plt.xticks(fontsize=20); plt.yticks(fontsize=20);
                    plt.title(name+' at '+str(round(time/1.0e-15,6))+' fs',fontdict=font)
                    ...
                    den = data['Derived/Number_Density/'+name[0:-8]].data/denunit/0.96
                    ...
                    den = data['Derived/Average_Particle_Energy/'+name[0:-6]].data/(q0*1.0e6)
                    if np.min(den.T) == np.max(den.T):
                        continue
