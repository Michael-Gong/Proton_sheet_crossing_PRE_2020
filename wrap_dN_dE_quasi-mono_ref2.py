...
if __name__ == "__main__":
  ...
  dist_x1, den1 = pxpy_to_energy(ek,ww)
  plt.plot(dist_x1,den1,color='red',linewidth=3,label='$\sigma_0=1\mu m;\ a_0=122$')

  data = sdf.read('./PW_a120_w020/'+str(13).zfill(4)+".sdf",dict=True)
  time=data['Header']['time']
  px = data['Particles/Px/'+name].data/(mass*m0*v0)
  py = data['Particles/Py/'+name].data/(mass*m0*v0)
  grid_y = data['Grid/Particles/'+name].data[1]/1e-6
  ...
  ww=ww[(px>0)&(abs(grid_y)<2)&(abs(theta)<30)]
  dist_x1, den1 = pxpy_to_energy(ek,ww)
  plt.plot(dist_x1,den1,color='green',linewidth=3,label='$\sigma_0=2\mu m;\ a_0=122$')
...
#  ww=ww[(px>0)&(abs(grid_y)<2)&(abs(theta)<30)]
#  dist_x1, den1 = pxpy_to_energy(ek,ww)
#  plt.plot(dist_x1,den1,color='mediumblue',linewidth=3,label='$\sigma_0=4\mu m;\ a_0=31$')

  data = sdf.read('./PW_a120_w040/'+str(n).zfill(4)+".sdf",dict=True)
  time=data['Header']['time']
  px = data['Particles/Px/'+name].data/(mass*m0*v0)
  py = data['Particles/Py/'+name].data/(mass*m0*v0)
  grid_y = data['Grid/Particles/'+name].data[1]/1e-6
  ...
  ww=ww[(px>0)&(abs(grid_y)<2)&(abs(theta)<30)]
  dist_x1, den1 = pxpy_to_energy(ek,ww)
  plt.plot(dist_x1,den1,color='blue',linewidth=3,label='$\sigma_0=4\mu m;\ a_0=122$')
...
#  ww=ww[(px>0)&(abs(grid_y)<2)&(abs(theta)<30)]
#  dist_x1, den1 = pxpy_to_energy(ek,ww)
#  plt.plot(dist_x1,den1,color='black',linewidth=3,label='$\sigma_0=12\mu m;\ a_0=10$')
...
#    plt.ylim(1e1,1e5)
  ...
  fig.savefig('wrap_dN_dE_quasi-mono_ref2.png',format='png',dpi=160)
  plt.close("all")
  print('wrap_dN_dE_quasi-mono_ref2.png')
